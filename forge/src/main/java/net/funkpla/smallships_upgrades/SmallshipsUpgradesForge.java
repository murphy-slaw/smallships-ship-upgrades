...
public class SmallshipsUpgradesForge {
  ...
  public SmallshipsUpgradesForge() {
    ...
    MinecraftForge.EVENT_BUS.addListener(this::handleLoad);
    UpgradePacketHandler.registerPackets();
  }
  ...
  public void onAttachingCapabilities(final AttachCapabilitiesEvent<Entity> event) {
    if (!(event.getObject() instanceof Ship)) return;
    UpgradeCapabilityAttacher.attach(event);
  }

  @SubscribeEvent
  public void handleLoad(PlayerEvent.StartTracking event) {
    if (event.getTarget() instanceof Upgradeable ship) {
      var upgrades = ship.getUpgrades();
      for (UpgradeType type : UpgradeType.values()) {
        UpgradePacketHandler.sendAddUpgradePacket(
            event.getTarget(), type, upgrades.getUpgradeCount(type));
      }
    }
  }
}
